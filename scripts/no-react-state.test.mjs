...
// grip-sudoku forbids React local state — all state lives in grips (see
...
const srcDirs = [join(root, 'src')].filter((d) => {
  try {
...
const APPROVALS = [
  // example: { id: 1, hook: 'useMemo', file: 'src/engine/sudoku.ts', reason: 'memoize solver search' },
];
...
for (const file of srcDirs.flatMap(walk)) {
  // Test files verify engine behavior, not UI state — and this script itself
  // scans only .ts/.tsx source, so excluding *.test.ts keeps the ban scoped
  // to application code.
...
if (violations.length) {
  console.error('FAIL: React local state hooks are banned in grip-sudoku (use grips). Found:');
